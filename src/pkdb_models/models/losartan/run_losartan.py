"""Tool to run the losartan model factory and simulation experiments."""
...
from pkdb_models.models.losartan import LOSARTAN_PATH
from pkdb_models.models.losartan.simulations import run_simulation_experiments, EXPERIMENTS
from sbmlutils.console import console

FACTORY_SCRIPT_PATH = LOSARTAN_PATH / "models" / "factory.py"
...
def _setup_custom_results_paths(results_dir: str):
    ...
    import pkdb_models.models.losartan as losartan
    custom_path = Path(results_dir).resolve()
    custom_path.mkdir(parents=True, exist_ok=True)
    # Override the module paths
    losartan.RESULTS_PATH = custom_path
    losartan.RESULTS_PATH_SIMULATION = custom_path / "simulation"
...
def _get_current_results_path():
    ...
    from pkdb_models.models.losartan import RESULTS_PATH
    return RESULTS_PATH
...
def main() -> None:
    ...
    if options.results_dir:
        ...
        if options.action and options.action.lower() == "factory":
            console.print(f"[cyan]Factory will use model base path: {losartan.MODEL_BASE_PATH}[/cyan]")
        else:
...
if __name__ == "__main__":
    """
    This script is intended to be run from the command line as 'run_losartan'.
    ...
    2. Run the Model Factory:
       ...
       $ run_losartan --action factory

    3. Run Simulations:
       List available experiments:
       $ run_losartan --action list_experiments

       Run all experiments:
       $ run_losartan --action simulate --experiments all

    5. Run Everything:
       Runs factory and all simulations.
       $ run_losartan --action all
       
       With custom results directory for figures:
       $ run_losartan --action all --results-dir '/path/to/my/results'
    """
    main()